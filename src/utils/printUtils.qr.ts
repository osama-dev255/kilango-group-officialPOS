...
export class PrintUtils {
  ...
  static printReceipt(transaction: any) {
    ...
    if (templateConfig.customTemplate) {
      ...
      // Generate QR code data (using transaction ID or receipt number)
      const qrData = `Receipt #: ${transaction.receiptNumber || Date.now()}\nDate: ${new Date().toLocaleDateString()}\nTotal: $${total.toFixed(2)}`;
        ...
        <html>
          ...
          <body>
            ...
            <div class="qr-section">
              ...
              <div class="qr-code">QR CODE<br/>RECEIPT<br/>#${transaction.receiptNumber || Date.now()}</div>
            </div>
  ...
  static printPurchaseReceipt(transaction: any) {
    ...
    if (templateConfig.customTemplate) {
      ...
      // Generate QR code data (using transaction ID or order number)
      const qrData = `Purchase Order #: ${transaction.orderNumber || 'PO-' + Date.now()}\nDate: ${new Date().toLocaleDateString()}\nTotal: $${total.toFixed(2)}`;
      ...
      receiptContent = `
        ...
        <html>
          ...
          <body>
            ...
            <div class="qr-section">
              <div class="qr-label">Scan for Details</div>
              <div class="qr-code">QR CODE<br/>ORDER<br/>#${transaction.orderNumber || 'PO-' + Date.now()}</div>
            </div>
