...
export class PrintUtils {
        ...
        <html>
          ...
          <body>
            ...
            <div class="footer">
              <div class="thank-you">Thank You!</div>
              <div>For more info, visit us at www.posbusiness.com</div>
              <div class="note">*Tax not included in total computation</div>
            </div>
          </body>
        </html>
      `;
  ...
  // Mobile-optimized printing method for purchase receipts with improved responsiveness
  static printPurchaseReceiptMobile(transaction: any) {
    // Check if there's already a print container to prevent multiple instances
    if (document.querySelector('#mobilePrintContainer')) {
      console.warn('Print operation already in progress');
      ...
      printContainer.innerHTML = `
        <div style="max-width: 320px; margin: 0 auto;">
            ...
            ${formattedItems.map((item: any) => `
              ...
              <div style="display: flex; margin-bottom: 5px;">
                <div style="flex: 1; text-align: right;">
                  ...
                  <span style="margin: 0 5px;">@</span>
                  <span>${item.price.toFixed(2)}</span>
                  <span style="font-weight: bold; margin-left: 10px;">${(item.price * item.quantity).toFixed(2)}</span>
                </div>
              </div>
            `).join('')}
          </div>
          ...
          <div style="border-top: 1px dashed #000; padding-top: 10px; margin-top: 10px;">
            ...
            <div style="font-size: 9px; font-style: italic; margin-top: 5px;">Note: Tax is for display purposes only and not included in calculations</div>
          </div>
  ...
  // Fallback printing method for purchase receipts when popups are blocked
  static printPurchaseReceiptFallback(transaction: any) {
    try {
      // Show loading indicator
      this.showLoadingIndicator('Preparing print...');
